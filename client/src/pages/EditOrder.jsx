  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setDeliveryFee(order.delivery || 0);
        setDiscountAmount(order.discount || 0);
        
        // Reverse calculate rates from amounts if subtotal exists
        const discSub = Math.max(0, order.subtotal - (order.discount || 0));
        if (discSub > 0) {
          if (order.gst !== undefined) setGstRate(Number(((order.gst / discSub) * 100).toFixed(2)));
          if (order.pst !== undefined) setPstRate(Number(((order.pst / discSub) * 100).toFixed(2)));
        }
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                {status === 'cancelled' && (
                  <p className="text-xs text-red-600 mt-2 font-medium">Warning: Saving as cancelled will restore all inventory.</p>
                )}
              </div>
